...
import { HexGrid, render, numberSetting, ProfileBadge } from '@boardzilla/core';
...
render(setup, {
  ...
  layout: game => {
    ...
    game.appearance({
      render: () => (
        <>
          <ProfileBadge player={game.players[0]}/>
          <ProfileBadge player={game.players[1]}/>
          <svg
    ...
    game.first(HexGrid)!.configureLayout({
      margin: 2,
      offsetRow: {x: 95, y: 0},
      offsetColumn: {x: 47.835, y: 70}
    })

    $.hex.all(Cell).appearance({
      aspectRatio: .866,
